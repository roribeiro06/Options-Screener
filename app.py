...
- **Requires a CONFIRMED earnings date** (unlike every other strategy here, which never spans one) -- no known catalyst, no candidate, regardless of how the rest of the math looks
- **Only the single expiration closest to (on or after) that earnings date** is used, not every later one that also happens to span it -- a catalyst trade should concentrate exposure around the event, not scatter near-duplicate rows across every weekly that comes after it. **Not bounded by the {sp.SPREAD_DTE_MIN}-{sp.SPREAD_DTE_MAX} DTE window above** -- a ticker whose earnings falls further out than {sp.SPREAD_DTE_MAX} days still gets a candidate at the nearest expiration after it, even though credit spreads/iron condor stay capped at {sp.SPREAD_DTE_MAX} -- but capped at **{sp.LONG_DTE_MAX} days**: earnings further out than that isn't worth tying up capital waiting for
