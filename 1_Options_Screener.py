...
st.caption("Every OPEN_POSITIONS entry's Max Loss AND contract premium change (today vs yesterday), "
           "cross-tabbed by sector -- **Tech** vs **Non-Tech**, via each ticker's yfinance GICS sector "
           "(Technology **and** Communication Services both count as Tech, since GOOG/META land in the "
           "latter under GICS but this app's own peer-correlation list already treats them as part of the "
           "same tech cluster as MSFT/AMZN/AAPL; ETFs like SMH have no sector and fall back to their "
           "category instead; SPCX/EWY/QQQ/AMZN are manually overridden to Tech -- see SECTOR_OVERRIDES "
           "in wheel_screener.py) -- by directional side, **Put** vs **Call** (put spreads join plain "
           "puts, call spreads join plain calls, not a separate Multi-Leg bucket -- a put spread is still "
           ...
           "- **Premium Change**: the contract's own price move, NOT Max Loss/risk -- e.g. an NVDA 230 "
           "put priced at \\$3.50 yesterday and \\$3.00 today is a -\\$0.50 change. Shown as \"$change "
           "(X%)\", same concise style as Max Loss. Single-leg: today's value = live ask (same basis as "
           "Open Positions' own CostToClose); yesterday's = that contract's own prevclose. Spread: both "
           "legs netted the SAME way the rest of the app already prices one to close -- today = short "
           "leg's ask minus long leg's bid; yesterday = short leg's prevclose minus long leg's prevclose. "
           "A position opened TODAY is excluded from yesterday's total (still counted in today's), so the "
           "two reflect what was actually held each day, not a hypothetical same-basket comparison.\n\n"
