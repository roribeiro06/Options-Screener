...
# position on the same ticker/side (see open_position_sides). Rolling is for
# LOSERS only -- a winner is left to ride, so it keeps blocking. A position
# stops blocking (so the ticker/side reappears as a roll candidate) only when
# BOTH hold: unrealized G/L is negative (live-priced, same ask-side worst-case
# CostToClose convention positions.py uses), AND the short strike's |delta|
# has reached ROLL_SHORT_DELTA. Trigger comes from the Options Alpha course
# (lesson 7, "Knowing When to Adjust"): short-strike delta 30 -- double the
# ~15 delta a position is typically sold at -- is the instructor's preferred
# trigger because it reflects real directional risk and tightens
# automatically with time decay; 40 gives more room. (Its 2x-credit trigger
# is deliberately NOT used: IV alone can trip it with no directional risk.)
# The course's net-credit-only rule for the roll itself can't be checked
# here, so it stays a manual check.
ROLL_SHORT_DELTA = 0.30
...
def _leg_at(chain, opt_type, strike):
    ...
    circular. Used only by _position_cost_and_short_delta below."""
    for o in chain:
...
def _position_cost_and_short_delta(pos):
    """(live worst-case ask-side cost to close one share, short leg's |delta|)
    for this OPEN_POSITIONS entry -- cost uses the same convention
    positions.py's evaluate_position uses for CostToClose; the short leg is
    the single leg for a put/call, the sold leg for a spread. Returns None if
    it can't be priced right now (chain unavailable, contract not found,
    delta missing) -- "can't tell," never a free pass."""
    kind = pos["type"]
    chain = td_chain(pos["ticker"], pos["expiration"])
    if not chain:
        return None
    if kind in ("put", "call"):
        short_leg = _leg_at(chain, kind, pos["strike"])
        if not short_leg:
            return None
        cost = short_leg["ask"] or 0
    else:
        ...
        if not (short_leg and long_leg):
            return None
        cost = (short_leg["ask"] or 0) - (long_leg["bid"] or 0)
    delta = short_leg.get("delta")
    if delta is None:
        return None
...
def _position_roll_eligible(pos):
    ...
    if not entry_credit:
        return False
    priced = _position_cost_and_short_delta(pos)
    if priced is None:
        return False
...
def open_position_sides(symbol):
    ...
    puts/put spreads, but INTC call spreads still show. A LOSING position
    under enough pressure (negative G/L and short delta >= ROLL_SHORT_DELTA,
    see _position_roll_eligible) no longer counts as occupying its side, so
    it can appear on the screener again as a roll candidate; winners always
    keep blocking. Contract Lookup deliberately ignores this entirely --
